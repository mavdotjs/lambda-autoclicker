...
<div role="tablist" class="tabs tabs-boxed {props.class}">
	{#each Object.entries(tabs) as [tabID]}
		...
		<a class:cursor-not-allowed={disabled} class:pointer-events-none={disabled} class:tab-disabled={disabled} role="tab" class="tab uppercase" onclick={() => tab = tabID} class:tab-active={tabID == tab}>{map?map[tabID]:tabID}</a>
	{/each}
</div>
